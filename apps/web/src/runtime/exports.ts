...
export function exportAsFigmaHtml(html: string, title: string): void {
  const doc = buildSrcdoc(buildFigmaInsertHtml(html));
  const blob = new Blob([doc], { type: 'text/html;charset=utf-8' });
  triggerDownload(blob, `${safeFilename(title, 'artifact')}-figma.html`);
}

export function exportAsFigmaZip(html: string, title: string): void {
  const slug = safeFilename(title, 'artifact');
  const slideCount = countHtmlSlides(html);
  const entries = [
    {
      path: `${slug}-figma/figma-insert.html`,
      content: buildSrcdoc(buildFigmaInsertHtml(html)),
    },
    {
      path: `${slug}-figma/README.md`,
      content:
        `# ${title || slug} Figma HTML\n\n` +
        `Use figma-insert.html first. It stacks all .slide frames vertically at 1280x720.\n` +
        `If your Figma import tool captures only the current viewport, import the files under figma-slides/ one by one.\n`,
    },
  ];
  for (let i = 0; i < slideCount; i += 1) {
    entries.push({
      path: `${slug}-figma/figma-slides/slide-${String(i + 1).padStart(2, '0')}.html`,
      content: buildSrcdoc(buildFigmaSingleSlideHtml(html, i)),
    });
  }
  const blob = buildZip(entries);
  triggerDownload(blob, `${slug}-figma.zip`);
}
