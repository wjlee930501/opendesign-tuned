...
export function ProjectView({
  ...
  const handleExportAsPptx = useCallback(
    (fileName: string) => {
      ...
      const prompt =
        `Export @${fileName} as a visual-faithful editable PPTX file titled "${baseTitle}".\n\n` +
        `**Generate.** Use the hybrid editable method by default: render each HTML slide ` +
        `twice at 1280x720 with Playwright/Chromium. First capture a background render with ` +
        `visible text hidden or made transparent, preserving layout, panels, charts, SVG, ` +
        `images, borders, and color. Then collect the rendered text boxes and overlay them ` +
        `as real editable PowerPoint text boxes in the same positions. This should look as ` +
        `close as possible to the HTML while keeping text editable. Do not create a ` +
        `screenshot-only deck, and do not attempt a pure manual shape reconstruction unless ` +
        `the user explicitly asks for lower-fidelity fully-vector output.\n\n` +
        `**Layout discipline.** Find the slide containers (usually \`.slide\` sections), ` +
        `disable CSS transitions/animations, set the viewport to exactly 1280x720, show one ` +
        `slide at a time, and map browser pixel coordinates into a 13.333x7.5 inch 16:9 ` +
        `PowerPoint canvas. For every meaningful text element, preserve text content, font ` +
        `size, weight, italic style, color, alignment, and approximate line height. Prefer ` +
        `element-level text boxes over per-word/per-character fragmentation.\n\n` +
        `**Verify (mandatory gate).** After writing, verify the PPTX has the expected slide ` +
        `count, 16:9 dimensions, one non-text background image per slide, and many editable ` +
        `text boxes. Report slide count, editable text box count, picture count, and file ` +
        `size. If the renderer is unavailable, say that explicitly instead of producing a ` +
        `fake editable deck.\n\n` +
        `Save into the current project folder (this conversation's working directory) as ` +
        `\`${baseTitle}.pptx\`. Report the on-disk path and a 1-line fidelity summary when done.`;
