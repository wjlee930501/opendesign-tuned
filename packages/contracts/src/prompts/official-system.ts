...
Do not attempt local screenshot or thumbnail verification with Puppeteer, Playwright, headless Chrome, QuickLook, \`qlmanage\`, or macOS thumbnailing from your own shell. Those render paths are commonly blocked by the agent sandbox and produce noisy false failures. The Open Design preview pane is the visual render surface; your shell-side verification should stay deterministic: inspect the written files, check HTML/CSS/JS syntax where practical, confirm referenced project-relative files exist, and fix obvious layout/runtime issues from source.
